...
class AIConfig(BaseModel):
    ...
    default_openai_model: str = Field(default="gpt-4o-mini", description="Default OpenAI model")
    default_anthropic_model: str = Field(default="claude-3-5-sonnet-20240620", description="Default Anthropic model")
    max_tokens_default: int = Field(default=1000, ge=1, le=8000, description="Default max tokens")
...
class RateLimitConfig(BaseModel):
    ...
    base_delay: float = Field(default=1.0, ge=0.1, le=10.0, description="Base delay in seconds")
    ...
    jitter_max: float = Field(default=0.1, ge=0.0, le=1.0, description="Maximum jitter fraction")
    rate_limit_delay: float = Field(default=60.0, ge=1.0, le=3600.0, description="Rate limit delay")
    quota_exceeded_delay: float = Field(default=3600.0, ge=60.0, le=86400.0, description="Quota exceeded delay")
...
class AsyncConfig(BaseModel):
    ...
    worker_timeout: int = Field(default=300, ge=30, le=3600, description="Worker timeout")
    job_timeout: int = Field(default=3600, ge=60, le=86400, description="Job timeout")
    max_chunk_size: int = Field(default=4000, ge=1000, le=10000, description="Max chunk size for transcripts")
    chunk_overlap: int = Field(default=200, ge=50, le=1000, description="Chunk overlap size")
...
class LoggingConfig(BaseModel):
    ...
    max_file_size: int = Field(default=10485760, ge=1048576, le=104857600, description="Max log file size (10MB)")
    ...
    use_structured: bool = Field(default=False, description="Use structured JSON logging")
...
class ApplicationConfig(BaseSettings):
    ...
    # User settings
    default_assignee: str = Field(default="Meeting Agent User", description="Default task assignee")
...
class ConfigManager:
    ...
    def _build_config_dict(cls) -> Dict[str, Any]:
        ...
        config_data["debug"] = os.getenv("DEBUG", "false").lower() in ("true", "1", "yes")
        config_data["default_assignee"] = os.getenv("DEFAULT_ASSIGNEE", "Meeting Agent User")
        ...
        config_data["ai"] = {
            ...
            "default_anthropic_model": os.getenv("DEFAULT_ANTHROPIC_MODEL", "claude-3-5-sonnet-20240620"),
        ...
        config_data["memory"] = {
            ...
            "enabled": os.getenv("MEMORY_ENABLED", "false").lower() in ("true", "1", "yes"),
        ...
        config_data["redis"] = {
            ...
            "socket_keepalive": os.getenv("REDIS_SOCKET_KEEPALIVE", "true").lower() in ("true", "1", "yes")
        }
        ...
        config_data["rate_limit"] = {
            ...
            "jitter": os.getenv("RATE_LIMIT_JITTER", "true").lower() in ("true", "1", "yes"),
            "jitter_max": float(os.getenv("RATE_LIMIT_JITTER_MAX", "0.1")),
            "rate_limit_delay": float(os.getenv("RATE_LIMIT_RATE_LIMIT_DELAY", "60.0")),
            "quota_exceeded_delay": float(os.getenv("RATE_LIMIT_QUOTA_EXCEEDED_DELAY", "3600.0"))
        }
        
        # Async configuration
        config_data["async_config"] = {
            "enabled": os.getenv("ENABLE_ASYNC_PROCESSING", "false").lower() in ("true", "1", "yes"),
        ...
        config_data["logging_config"] = {
            ...
            "use_rich": os.getenv("LOG_USE_RICH", "true").lower() in ("true", "1", "yes"),
            "use_structured": os.getenv("LOG_USE_STRUCTURED", "false").lower() in ("true", "1", "yes")
        }
