...
from .config import DEFAULT_USER_ID, MEM0_API_KEY
...
class MemoryClient:
    ...
    def store_user_preference(
        self, preference_type: str, preference_value: str, user_id: str = None
    ) -> None:
        """
    ...
    def store_task_feedback(
        self, task_info: Dict, feedback: str, user_id: str = None
    ) -> None:
        """
    ...
    def get_user_preferences(self, user_id: str = None) -> Dict:
        ...
        try:
            ...
            results = self.memory.search(
                query="User prefers", user_id=user_id, limit=20
            )
    ...
    def get_meeting_history_context(
        self, meeting_title: str, user_id: str = None
    ) -> List[Dict]:
        """
        ...
        try:
            ...
            results = self.memory.search(
                query=f"Meeting similar to {meeting_title}", user_id=user_id, limit=10
            )

            # Filter for meeting-related memories
            meeting_contexts = []
            for result in results:
                if result.get("metadata", {}).get("category") in [
                    "meeting_semantic",
                    "meeting_episodic",
                ]:
    ...
    def learn_from_interaction(
        self, interaction_data: Dict, user_id: str = None
    ) -> None:
        """
            ...
            if interaction_data.get("user_feedback"):
                ...
                self.memory.add(
                    ...
                    metadata={
                        ...
                        "feedback_type": interaction_data.get(
