...
class TaskManager:
    ...
    def create_tasks_from_action_items(self, action_items: List[str], meeting_id: str, default_due_date: str = None) -> List[str]:
        ...
        for action in action_items:
            ...
            try:
                ...
                if self.memory_client and self.memory_client.is_enabled():
                    ...
                    self.memory_client.store_task_feedback(task_info, "Task created successfully from action item")
            except Exception as e:
                print(f"✗ Error creating task '{task_desc}': {e}")
        
        return task_ids
    
    def create_selected_action_items(self, action_items: List[str], meeting_id: str) -> List[str]:
        ...
        while True:
            choice = input("Select action items (comma-separated numbers, e.g., '1,3,4' or 'all'): ").strip()
            ...
            try:
                ...
                for num in choice.split(','):
                    ...
                    if 0 <= num < len(action_items):
                        ...
                        print(f"Invalid number: {num + 1}. Please use numbers 1-{len(action_items)}.")
                        break
                else:
                    break
            except ValueError:
        ...
        while True:
            ...
            try:
                ...
                if self.memory_client and self.memory_client.is_enabled():
                    ...
                    self.memory_client.store_task_feedback(task_info, "Task created successfully from custom input")
            except Exception as e:
                print(f"✗ Error creating task '{task_input['description']}': {e}")
        
        return task_ids
    
    def handle_task_creation(self, notes: str, meeting_id: str, meeting_title: str, original_question: str = None) -> None:
        ...
        if action_items:
            ...
            if self.ui.should_create_from_actions(len(action_items)):
                task_ids.extend(
                    self.create_tasks_from_action_items(action_items, meeting_id, default_due_date)
                )
        else:
    ...
    def suggest_and_create_tasks(self, notes: str, meeting_id: str, meeting_title: str) -> None:
                ...
                try:
                    ...
                    for num in choice.split(','):
                        ...
                        if selected_indices:
                            selected_actions = [action_items[i] for i in selected_indices]
                            task_ids.extend(
                                self.create_tasks_from_action_items(selected_actions, meeting_id)
                            )
                        else:
                            print("No valid action items selected.")
                except ValueError:
                    print("Invalid input format. Please use 'all' or numbers like '1,3,4'.")
            else:
    ...
    def create_tasks_from_suggestions(self, suggested_tasks: List[Dict], meeting_id: str) -> List[str]:
        ...
        for suggestion in suggested_tasks:
            ...
            due_date = self.ui.get_task_due_date_with_suggestion(task_desc, suggested_due)
            ...
            try:
                ...
                if self.memory_client and self.memory_client.is_enabled():
                    ...
                    self.memory_client.store_task_feedback(task_info, "Task created successfully from AI suggestion")
            except Exception as e:
                print(f"✗ Error creating task '{task_desc}': {e}")
    ...
    def is_task_related_question(self, question: str) -> bool:
        ...
        task_keywords = [
            ...
            "add them to task", "add to task", "make task", "turn into task",
            "convert to task", "task these", "task them", "add as task"
        ]
