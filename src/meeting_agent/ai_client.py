...
from .ai_config import TaskType, get_ai_config
from .config import ANTHROPIC_CLIENT, OPENAI_CLIENT
from .rate_limiter import APIProvider, get_rate_limiter
...
class AIClient:
    ...
    def check_similarity(
        self, new_notes: str, past_meetings: List[dict], new_page_id: str
    ) -> List[str]:
        ...
        for meeting in past_meetings:
            ...
            desc = ""
            if (
                "Description" in meeting["properties"]
                and meeting["properties"]["Description"]["rich_text"]
            ):
                desc = meeting["properties"]["Description"]["rich_text"][0]["text"][
                    "content"
                ]
    ...
    def process_queued_requests(self, max_requests: int = 10) -> dict:
        ...
        return {
            "openai_processed": self.rate_limiter.process_queued_requests(
                APIProvider.OPENAI, max_requests
            ),
            "anthropic_processed": self.rate_limiter.process_queued_requests(
                APIProvider.ANTHROPIC, max_requests
            ),
        }
