...
class MeetingAgent(AsyncProcessingMixin):
    ...
    def __init__(self):
        ...
        self.task_manager = TaskManager(
            self.notion_client, self.ai_client, self.ui, self.memory_client
        )
        ...
        super().__init__()
        self.enable_async = (
            os.getenv("ENABLE_ASYNC_PROCESSING", "false").lower() == "true"
        )
    ...
    def run(self):
        ...
        if should_chunk:
            ...
            print(
                f"📝 Estimated processing time: {self._estimate_processing_time(transcript_size)} seconds"
            )
        ...
        self.notion_client.update_meeting_fields(
            new_page_id, meeting_type, tags, status
        )
        ...
        if similar_ids:
            ...
            similar_meetings = [
                self.notion_client.get_meeting_details(sim_id) for sim_id in similar_ids
            ]
        ...
        while True:
            question = self.ui.get_user_input(
                "Ask a question about meetings (or 'exit'): "
            )
            ...
            if self.task_manager.is_task_related_question(question):
                self.task_manager.handle_task_creation(
                    notes, new_page_id, title, question
                )
            else:
                ...
                memory_context = ""
                if self.memory_client.is_enabled():
                    relevant_memories = self.memory_client.get_relevant_context(
                        question, limit=3
                    )
    ...
    def _process_transcript_async(
        self, transcript: str, title: str, date: str
    ) -> tuple:
        """Process transcript asynchronously using worker queue"""
        print("\n🚀 Submitting transcript for async processing...")

        meeting_data = {"title": title, "date": date}
    ...
    def _display_rate_limit_status(self):
        ...
        for provider_name, provider_status in status.items():
            ...
            if provider_status["limit_requests"]:
                remaining_pct = (
                    provider_status["remaining_requests"]
                    / provider_status["limit_requests"]
                ) * 100
                status_emoji = (
                    "🟢" if remaining_pct > 50 else "🟡" if remaining_pct > 20 else "🔴"
                )
                print(
                    f"  Request quota: {status_emoji} {provider_status['remaining_requests']}/{provider_status['limit_requests']} ({remaining_pct:.1f}%)"
                )

            if provider_status["limit_tokens"]:
                remaining_pct = (
                    provider_status["remaining_tokens"]
                    / provider_status["limit_tokens"]
                ) * 100
                status_emoji = (
                    "🟢" if remaining_pct > 50 else "🟡" if remaining_pct > 20 else "🔴"
                )
                print(
                    f"  Token quota: {status_emoji} {provider_status['remaining_tokens']:,}/{provider_status['limit_tokens']:,} ({remaining_pct:.1f}%)"
                )
        ...
        total_processed = (
            processed["openai_processed"] + processed["anthropic_processed"]
        )
